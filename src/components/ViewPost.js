...
class ViewPost extends Component {
  ...
  componentWillReceiveProps = newProps => {
    ...
    if (!_.isEmpty(newProps.categories) && !_.isEmpty(newProps.posts)) {
      let post = newProps.posts.filter(
  ...
  render() {
    ...
    if (_.isEmpty(categories)) {
      return <div className="loading-post" />;
      ...
      return (
        <div className="home-view">
          <span onClick={() => this.props.deletePost(post.id)}> X </span>
          &nbsp;&nbsp;
          ...
          <div>
            <span onClick={() => this.changeVote(post.id, 'downVote')}>
              {' '}V{' '}
            </span>
          ...
          >
            {isPostModalOpen &&
              <EditPost
                post={post}
                closeModal={() => this.setState({ isPostModalOpen: false })}
              />}
          </Modal>
        </div>
      );
