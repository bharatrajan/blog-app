    ...
    if (_.isEmpty(comment) || comment.deleted) {
      ...
      return (
        <div className="comment-card">
          ...
          <span onClick={() => this.setState({ isCommentModalOpen: true })}>
            {' '}EDIT{' '}
          </span>
          ...
          <div>
            {' '}Created at: {util.ts2Time(comment.timestamp)}
          </div>
          <div>
            {' '}Vote : {comment.voteScore}
          </div>
          <div>
            <span onClick={() => this.changeVote(comment.id, 'downVote')}>
              {' '}V{' '}
            </span>
            <span onClick={() => this.changeVote(comment.id, 'upVote')}>
              {' '}^{' '}
            </span>
          </div>
          <br />
          ...
          >
            {isCommentModalOpen &&
              <EditComment
                comment={comment}
                closeModal={() => this.setState({ isCommentModalOpen: false })}
              />}
          </Modal>
        </div>
      );
